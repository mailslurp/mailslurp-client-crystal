...
module 
  class AliasControllerApi
    ...
    def get_alias_threads_with_http_info(alias_id : String, page : Int32?, size : Int32?, sort : String?, since : Time?, before : Time?)
      ...
      # return_type
      return_type = "PageAliasThreadProjection"
      ...
      return PageAliasThreadProjection.from_json(data), status_code, headers
    end

    # Get all email aliases you have created
    # Get all email aliases in paginated form
    # @return [PageAlias]
    def get_aliases(search : String?, page : Int32?, size : Int32?, sort : String?, since : Time?, before : Time?)
      data, _status_code, _headers = get_aliases_with_http_info(search, page, size, sort, since, before)
      data
    end
    ...
    def get_aliases_with_http_info(search : String?, page : Int32?, size : Int32?, sort : String?, since : Time?, before : Time?)
      if @api_client.config.debugging
    ...
    def get_thread_with_http_info(thread_id : String)
      if @api_client.config.debugging
        Log.debug {"Calling API: AliasControllerApi.get_thread ..."}
      end
      # verify the required parameter "thread_id" is set
      if @api_client.config.client_side_validation && thread_id.nil?
        raise ArgumentError.new("Missing the required parameter 'thread_id' when calling AliasControllerApi.get_thread")
      end
      # resource path
      local_var_path = "/aliases/threads/{threadId}".sub("{" + "threadId" + "}", URI.encode(thread_id.to_s))
      ...
      # return_type
      return_type = "AliasThreadProjection"
      ...
      data, status_code, headers = @api_client.call_api(:GET,
                                                        ...
                                                        form_params)
      if @api_client.config.debugging
        Log.debug {"API called: AliasControllerApi#get_thread\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"}
      end
      return AliasThreadProjection.from_json(data), status_code, headers
    end

    # Get all threads
    # Returns threads created for all aliases in paginated form
    # @return [PageAliasThreadProjection]
    def get_threads_paginated(page : Int32?, size : Int32?, sort : String?, since : Time?, before : Time?)
      data, _status_code, _headers = get_threads_paginated_with_http_info(page, size, sort, since, before)
      data
    end
    ...
    # Returns threads created for all aliases in paginated form
    # @return [Array<(PageAliasThreadProjection, Integer, Hash)>] PageAliasThreadProjection data, response status code and response headers
    def get_threads_paginated_with_http_info(page : Int32?, size : Int32?, sort : String?, since : Time?, before : Time?)
      if @api_client.config.debugging
        Log.debug {"Calling API: AliasControllerApi.get_threads_paginated ..."}
      end
      allowable_values = ["ASC", "DESC"]
      if @api_client.config.client_side_validation && sort && !allowable_values.includes?(sort)
        raise ArgumentError.new("invalid value for \"sort\", must be one of #{allowable_values}")
      ...
      data, status_code, headers = @api_client.call_api(:GET,
                                                        local_var_path,
                                                        :"AliasControllerApi.get_threads_paginated",
                                                        return_type,
                                                        post_body,
                                                        auth_names,
                                                        header_params,
                                                        ...
                                                        form_params)
      if @api_client.config.debugging
        Log.debug {"API called: AliasControllerApi#get_threads_paginated\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"}
      end
      return PageAliasThreadProjection.from_json(data), status_code, headers
    end
