...
module 
  class TemplateControllerApi
    ...
    def get_all_templates(before : Time?, page : Int32?, since : Time?, size : Int32?, sort : String?)
      data, _status_code, _headers = get_all_templates_with_http_info(before, page, since, size, sort)
      data
    end

    # Get all Templates in paginated format
    # @return [Array<(PageTemplateProjection, Integer, Hash)>] PageTemplateProjection data, response status code and response headers
    def get_all_templates_with_http_info(before : Time?, page : Int32?, since : Time?, size : Int32?, sort : String?)
      if @api_client.config.debugging
