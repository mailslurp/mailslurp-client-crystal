...
module 
  class ContactControllerApi
    ...
    def get_all_contacts(before : Time?, page : Int32?, since : Time?, size : Int32?, sort : String?)
      data, _status_code, _headers = get_all_contacts_with_http_info(before, page, since, size, sort)
      data
    end

    # Get all contacts
    # @return [Array<(PageContactProjection, Integer, Hash)>] PageContactProjection data, response status code and response headers
    def get_all_contacts_with_http_info(before : Time?, page : Int32?, since : Time?, size : Int32?, sort : String?)
      if @api_client.config.debugging
