...
module 
  class MissedEmailControllerApi
    ...
    # @param timeout [Int64] Optional timeout milliseconds
    # @return [MissedEmail]
    def wait_for_nth_missed_email(inbox_id : String, timeout : Int64, index : Int32?)
      data, _status_code, _headers = wait_for_nth_missed_email_with_http_info(inbox_id, timeout, index)
      data
    end
    ...
    # @param timeout [Int64] Optional timeout milliseconds
    # @return [Array<(MissedEmail, Integer, Hash)>] MissedEmail data, response status code and response headers
    def wait_for_nth_missed_email_with_http_info(inbox_id : String, timeout : Int64, index : Int32?)
      if @api_client.config.debugging
        Log.debug {"Calling API: MissedEmailControllerApi.wait_for_nth_missed_email ..."}
      end
      # verify the required parameter "inbox_id" is set
      if @api_client.config.client_side_validation && inbox_id.nil?
        raise ArgumentError.new("Missing the required parameter 'inbox_id' when calling MissedEmailControllerApi.wait_for_nth_missed_email")
      end
      # verify the required parameter "timeout" is set
      if @api_client.config.client_side_validation && timeout.nil?
        raise ArgumentError.new("Missing the required parameter 'timeout' when calling MissedEmailControllerApi.wait_for_nth_missed_email")
      end
      # resource path
      local_var_path = "/missed-emails/waitForNthMissedEmail".sub("{" + "inboxId" + "}", URI.encode(inbox_id.to_s)).sub("{" + "timeout" + "}", URI.encode(timeout.to_s))
